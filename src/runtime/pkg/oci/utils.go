...
const (
	...
	crioPodLinuxResourcesKey           = "pod-linux-resources.crio.io"
	crioPodLinuxResourcesKeyDeprecated = "io.kubernetes.cri-o.PodLinuxResources"
)
...
func CalculateSandboxSizing(spec *specs.Spec) (numCPU float32, memSizeMB uint32) {
	...
	if period == 0 && quota == 0 && memory == 0 {
		crioAnnotation := ""
		if annotationNew, ok := spec.Annotations[crioPodLinuxResourcesKey]; ok {
			crioAnnotation = annotationNew
		} else if annotationOld, ok := spec.Annotations[crioPodLinuxResourcesKeyDeprecated]; ok {
			crioAnnotation = annotationOld
		}
		if crioAnnotation != "" {
			var podResources crioPodLinuxResources
			if err := json.Unmarshal([]byte(crioAnnotation), &podResources); err == nil {
				...
				if podResources.CPUShares > 0 {
					shares = uint64(podResources.CPUShares)
				}
